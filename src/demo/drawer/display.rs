...
pub(super) fn plugin(app: &mut App) {
    app.add_systems(Startup, setup_drawer)
        .add_systems(Update, (show_drawer, hide_drawer).in_set(AppSet::Update));
}
...
fn slide_drawer(mut drawer_query: Query<(&mut Transform, &LetterDrawer)>, time: Res<Time>) {
    if let Ok((mut transform, drawer)) = drawer_query.get_single_mut() {
        transform.translation += drawer.target_position.extend(0.0) + time.delta_seconds();
    }
}
